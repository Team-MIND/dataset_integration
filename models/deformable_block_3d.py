...
class DeformConv3d(nn.Module):
    ...
    def forward(self, x):
        if self.mode == "deformable":
            offset = self.offset_conv_kernel(x) #derive from input
        else:
            b, c, h, w, d = x.size()
        ...
        # (b, 3N, h, w, d)
        ''' 
        offsets + coordiante map input + kernel coords -> coords of offset kernels
        select all values from each batches and activation by the stride increments
        '''
        p = self._get_p(offset, dtype)
        p = p[:, :, ::self.stride, ::self.stride, ::self.stride] #we want to visualize p, so make a copy for showing later 
...
#         q_sss = p.data.floor() - same? / torch.Tensor(p.data).floor()
        ...
        '''
        select kernel dim *3 points in each batch along the height, width, depth coords 
        take ceil / floor typers -> clamp coords to input space
        create all combinations of floor/ceil types along all the dimentions
        take the combinations and cat all the dimentions togeather
        do this for all batches (thats what the ... implies)
        '''
        q_sss = torch.cat([
        ...
        # (b, h, w, d, N)
        '''
        create boolean mask of all N wide entries for h,w,d to ensure legal points when they are used to
        get real values 
        '''
        mask = torch.cat([
        ...
        '''
        from the original point and the offset kernel points merge in a permutative fashion 
        given the size of the kernels for each part of it hense the N width selections
        '''  
        ...
        '''
        given the coordiantes of the offset kernel points and the input
        get the values from the input at specified points
        '''
    ...
    def _get_p_n(self, N, dtype): #get the grid coords of all the kernels (treats like normal conv)
    ...
    @staticmethod
    def _get_p_0(h, w, d, N, dtype): #gets coord grid for the total input 
    ...
    def _get_p(self, offset, dtype): #make offsets into actual coords on the input
    ...
    def _get_x_q(self, x, q, N):
        ...
        #create indecies for all input of our data
        # via normal kernal width selection size 
